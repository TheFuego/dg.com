...
function addToCart(x) {
    ...
    const myInt = setInterval(function () {
        if (opacity > 0) {
            height += 2
            opacity -= 0.15
            tooltipClass[featured.indexOf(x)].style.marginTop = `-${height}px`
            tooltipClass[featured.indexOf(x)].style.opacity = `${opacity}`
        }if (opacity <= 0) {
            clearInterval(myInt)
            
            height = 0
            opacity = 1
            tooltipClass[featured.indexOf(x)].innerHTML = 'Must choose a size'
            tooltipClass[featured.indexOf(x)].classList.remove('tooltip-bought')
            tooltipClass[featured.indexOf(x)].style.marginTop = null
            tooltipClass[featured.indexOf(x)].style.opacity = null
        }
